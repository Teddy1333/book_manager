...
def convert_audio_to_wav(audio_bytes: bytes, suffix: str = ".wav") -> bytes:
    ...
    try:
        ...
        command = [ffmpeg_path, "-y", "-i", str(input_path), "-ac", "1", "-ar", "16000", "-f", "wav", str(output_path)]
...
def transcribe_audio(audio_bytes: bytes, suffix: str = ".wav", language: str = "bg-BG") -> str:
    if not audio_bytes:
        return ""
    ...
    try:
        ...
        with tempfile.NamedTemporaryFile(delete=False, suffix=output_suffix) as temp_file:
